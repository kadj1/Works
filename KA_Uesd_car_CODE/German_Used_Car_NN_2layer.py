...
with open('autos_Indexing.csv', "r") as rf:
    for i in rf.readlines()[2:100001]:
        train_x_str.append(i.split(',')[:12])
with open('autos_Indexing.csv', "r") as rf:
    for i in rf.readlines()[2:100001]:
        train_y_str.append(i.split(',')[12:13])
with open('autos_Indexing.csv', "r") as rf:
    for i in rf.readlines()[100001:110001]:
...
W2 = tf.get_variable("W2", shape = [256,1], initializer = xavier_init(256,1))
...
for s in range(epoch):
    cost_val, hy_val, _ = sess.run([cost, hypothesis, train],feed_dict = {X:data_x,Y:data_y,dropout_rate : 1.0})
    if s % int((epoch/10)) == 0:
